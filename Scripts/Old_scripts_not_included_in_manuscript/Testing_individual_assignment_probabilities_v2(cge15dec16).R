...
instructdat_k2=read.table("/Users/chriseckert/Dropbox/Shared/AndyWong/1. Hierarchical Structure/Maggie_Hierarchy/Figures/Figure_3_Instruc_w_map/acanadensis_K2.indq", header=FALSE, stringsAsFactors = FALSE)
...
Fst=read.csv("/Users/chriseckert/Dropbox/Shared/AndyWong/1. Hierarchical Structure/Maggie_Hierarchy/data/Hierstruct_Fst matrix.csv", header=TRUE)
...
geno08_mpb=read.csv("/Users/chriseckert/Dropbox/Shared/AndyWong/1. Hierarchical Structure/Maggie_Hierarchy/data/Hierstruct_microsat.csv", header=TRUE, stringsAsFactors = FALSE)
...
hist(instructdat_k2_assign_w_plots$assign_blue_group_arc_sin)
summary(instructdat_k2_assign_w_plots$assign_blue_group_arc_sin)


#there are no individuals with 100% or 0% assignment to blue/cluster 1
hist(instructdat_k2_assign_w_plots$assign_blue_group)
summary(instructdat_k2_assign_w_plots$assign_blue_group)
...
hist(instructdat_k2_assign_w_plots$assign_blue_group_logit)
summary(instructdat_k2_assign_w_plots$assign_blue_group_logit)
...
# WHY NOT JUST RUN A HIERARCHICAL LINEAR MODEL (LMER) ON LOGIT-TRANSFORMED ASSIGNMENT PROBABILITIES, AS PER THE FLORAL DATA
...
# How admixed is the average individual?

hist(instructdat_k2_assign_w_plots$assign_blue_group)
summary(instructdat_k2_assign_w_plots$assign_blue_group)
instructdat_k2_assign_w_plots$admix <- factor(ifelse(instructdat_k2_assign_w_plots$assign_blue_group >= 0.2 | instructdat_k2_assign_w_plots$assign_blue_group <=0.8, 1, 0))
instructdat_k2_assign_w_plots$admix <- factor(recode(instructdat_k2_assign_w_plots$assign_blue_group, " 0.2:0.8 = '1'; else = '0'"))
summary(instructdat_k2_assign_w_plots$admix)
169/(169+111)
...
#Extract variance components
summary(assign_prob_lmer_reml)

VarCorr(assign_prob_lmer_reml) #lists the standard deviations
VarCorr(assign_prob_lmer_reml)[1] #this gives varainces
(Plot<-as.numeric(VarCorr(assign_prob_lmer_reml)[1]))
(Outcrop<-as.numeric(VarCorr(assign_prob_lmer_reml)[2]))
(Plant<-(as.numeric(sigma(assign_prob_lmer_reml)))^2)

#Find proportion of variance attributed to each random effect
selcvars=c(Outcrop, Plot, Plant)
