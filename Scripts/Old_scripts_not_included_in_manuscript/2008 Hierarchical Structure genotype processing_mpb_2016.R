# This script is a hybrid between work MPB did to validate the data and scripts produced by AW. 
#AW comment:2008 genotype processing - for STRUCTURE, InStruct, Arlequin, HierFstat, and GStudio


#Geno08 is the data for the microsatellites amplified and scored by AW in 2008. The goal here is to characterize hierearchical genetic diversity among 7 outcrops of
...
#on a single randomly selected flower and each plant was genotyped for 9 microsatellite loci. For fine-scale genotypic diversity an additional locus (100.2-8) was screened. 

#280 plants in total, but note that the actual number of plants screened was 275.
...
#First AW removes data for locus 100.2-8. MPB found 2 microsat-plant genotype combos that were repeated in the data. MPB removed them after loading the data
...
str(geno08)
summary(geno08)
#location of AW's original data
#read.csv("~/Desktop/Current R actives/allgenotype2008.csv", header=TRUE)->geno08

#Let's also use dplyr to find which observations have duplicates
...
#Looks like AW just entered data for two samples in 4 times. 

#Let's get rid of these duplicates:
geno08=geno08[!(rownames(geno08) %in% c(522, 523,524, 526, 527, 528)),]
#here AW removes all the data for locus 100.2.8. There are 280 of these that are removed. This marker is only used to explore fine-scale genetic diversity. 
#The reason for exlcuding this locus from the higher scales (like between plot diversity) is not explained by AW. I would like to know why and include justification for its exlcusion in the manuscript.

#Here  I explore why this locus (100.2-8) was removed from the data
#the dataset geno_100.2_8 contanis data for the locus 100.2-8
geno_100.2_8= geno08 %>% filter(microsat=="100.2-8")
head(geno_100.2_8)
is.na(geno_100.2_8[geno_100.2_8$pop=="QRL1",])

geno_100.2_8[geno_100.2_8$pop=="QRL1" & is.na(geno_100.2_8$allele1),]
nrow(geno_100.2_8[geno_100.2_8$pop=="QRL1" & is.na(geno_100.2_8$allele1),])

nrow(geno_100.2_8[geno_100.2_8$pop=="QRL1",])

#Even for fine scale data 16 / 40 plants are missing data for locus 100.2.8

#First I check if the marker was monomorphic: Nope not monomorphic
hist(geno_100.2_8$allele1)
quartz()
par(mfrow=c(4,2))
for(i in unique(geno_100.2_8$pop)){
        hist(geno_100.2_8$allele1[geno_100.2_8$pop==i])
}
dev.off()
#Now I'm checking if it has missing data: Yes, lots of missing data
geno08[geno08$microsat=="100.2-8" & is.na(geno08$allele1),]
geno_100.2_8 %>% group_by(pop) %>% summarise(n=n())
geno_100.2_8 %>% group_by(pop,status) %>% summarise(n=n())
#how many missing genotypes are there by population and plot (AW calls plot quadrat), 
#I don't know if plate is the pcr plate or the plate which the pcr product was visualized on. 
missing_geno_100.2_8=geno_100.2_8[is.na(geno_100.2_8$allele1),]  %>% group_by(pop,quadrat,plate) 

temp=missing_geno_100.2_8%>% summarise(n=n()) 
#number of genotypes with no plate ID (I assume the pcr either failed or the reaction was never done)
temp %>% filter(is.na(plate))

#Conclusion on why locus 100.2-8 was removed. locus (100.2-8) was removed because it has a high rate of missing data. I don't know if this locus has missing data b/c reactions failed or b/c they were never run.
#many of the missing plant-marker combos are missing and the plate # is NA. If plate number refers to plate position of pcr plate this would suggest to me that 
#missing data is missing b/c the reaction was never done.

unique(geno08$pop)
str(geno08)


#Here AW removes locus 100.2-8

geno08=geno08[geno08$microsat!="100.2-8",]
nrow(geno08)


#Now he makes a dataset where status==0. I guess status==0 indicates failed reactions?
#I don't know what status 0.5 means.
...
# this is probably a coding error. Check if allele 208 is out of range for this population at this locus.
geno08NA[!(is.na(geno08NA$allele1)),]
length(geno08NA$allele1[is.na(geno08NA$allele1)])
#allele 208 is not out of range for this microsat. I assume this is a coding error and that this should not be status=0, 
#but for the sake of simplicitly I will assume AW judgement this genotype to be unreliable when scoring this genotype. 
hist(geno08$allele1[geno08$microsat=="50-7"],75)

#There 257 missing unique reactions (this includes the one discussed above where allele scores are available fo locus 50-7 but the status=0) 
dim(geno08NA) #257 missing unique reactions
geno08Good<-geno08[geno08$status!=0,]

#In what AW calls Good data there is also one row where the status==1 but the allele1 and allele2 are NA. Is this a data entry problem or something else?
geno08Good[is.na(geno08Good$allele1),]

#Here AW calculates the fraction of reactions that failed
#This calculation is wrong in that geno08Good has one reactions with status 1 but no allele info 
#and geno08NA has one site with info
(dim(geno08NA)[1]-1)/(dim(geno08Good)[1]-1) # this is still pretty high. 
#but the actual fraction should be calculated as:
(dim(geno08NA)[1]-1)/(dim(geno08Good)[1]-1) # this is still pretty high. 

#should have 2520 microsats screened: note that the original data set had 2 reactions which were replicated 4 times in the data. These extra observations were removed above. 
9*7*2*20
length(geno08$RxnID)
...
#I assume this is where AW replaces the failed reactions. #What doesn't make sense to me is what is 
#the column "Type".
#For the 2008 data, the options are 1, 2 and R. What do those categories mean?
#Also I think this dataset contains data for another project. AW only uses a subset here.
...
#Here MPB figures out what Type is kept in the data.
unique(replace$Type[replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg"])

#AW removes data for any year other than 2008, and only keeps data where Type is 1, 2, R or Reproductive. 
#I would like to know what these types are. 
nrow(replace[replace$Year=="2008" & replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg" & replace$microsat!="100.2-8",])

#AW did a total of 360 replacement reactions for the 2008 A.c hierarchy study.  
#Of these how many failed? 38, there is a genotype score for QBED1-3-50-7, but status ==0 because there are many many peaks
nrow(replace[replace$Year=="2008" & replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg" & replace$status==0 & replace$microsat!="100.2-8",])
nrow(replace[replace$Year=="2008" & replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg" & replace$status==0 & replace$microsat!="100.2-8" & !(is.na(replace$allele1)),])
replace[replace$Year=="2008" & replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg" & replace$status==0 & replace$microsat!="100.2-8" & !(is.na(replace$allele1)),]
38/360
360-38


#AW makes data with status>0, so reactions that worked
replace08_all<-replace[replace$Year=="2008" & replace$Type!="Seed" & replace$Type!="Seedling" & replace$Type!="V" & replace$Type!="Vegetative" & replace$Type!="Veg" &  replace$microsat!="100.2-8",]
...
dim(replace08) #AW :322 replacement reactions that worked. Seems a little low, but considering that we're missing 257 rxns in total, this is pretty good. Should just about do it. 


#why r there more replacements than missing data?
# I think some of these were already entered in the allgenotype2008.csv dataset:
# test this idea. 
nrow(replace08)

replace08$RxnID=paste(replace08$pop,replace08$plant,replace08$microsat,sep="-")
replace08_all$RxnID=paste(replace08_all$pop,replace08_all$plant,replace08_all$microsat,sep="-")

replace08$temp=paste(replace08$RxnID, as.character(replace08$allele1),  as.character(replace08$allele2),sep="_")
length(replace08$temp)
#now do the same thing for the geno
geno08$temp=paste(geno08$RxnID, as.character(geno08$allele1),as.character(geno08$allele2), sep="_")

#are there replacements already in the geno08 data?
nrow(replace08[replace08$temp %in% geno08$temp,])

#There are 64 in the replace data set that already exist in the geno08 dataset. 
#WHY? Were these reactions that were re-done or was this the first time these reactions were performed and 
#they were run along with a set that was being replaced?
360-64 #So 296 reactions were done and not yet recored in the allgenotype2008.csv dataset. 

#Of the 360 replacement reactions 38 did not work, 322 worked and 64 of the ones that worked were already recorded in the allgenotype2008.csv data
322-64

#Here MPB removes the data from the replace dataset that is already present in the allgenotype2008.csv dataset.  
replace08=replace08[!(replace08$temp %in% geno08$temp),]
nrow(replace08)

#of the 258 replacement reactions that worked there are no perfect duplicates
replace08[duplicated(replace08),]
replace08$pop_plant_microsat=paste(replace08$pop, replace08$plant, replace08$microsat, sep="_")
duplicated_replacements=replace08$pop_plant_microsat[duplicated(replace08$pop_plant_microsat)] 
        
        
replace08[replace08$pop_plant_microsat %in% duplicated_replacements,]

#It looks like AW either scored some genotypes twice or ran duplicate samples as a kind of control to test for repeatability of scoring (but that isn't quite right by running it on the same gel). 

#Remove the duplicates:
rownames(replace08[replace08$pop_plant_microsat %in% duplicated_replacements,])
#I remove 107 because it's a duplicate and its status=0.5 but the other sample has status==1 
#removed 609 because it's genotype was further from others for this microsat compared to the replicate sample
#607, 608 and 610 removed, but matching pair has near identical genotype (within 0.05)
replace08=replace08[!(rownames(replace08) %in%  c(107,607, 608,609,610)),]
#so there are actually only 253 replacement reactions.
#which 4 needed to be redone but were never redone?


#find out how many of the working 258 reactions that were not already recorded in the allgenotype2008.csv data had status 0.5
replace08[replace08$status==0.5,]
nrow(replace08)

#From the list of reactions that needed to be redone geno08NA how many of them are not in the replacement list?
#There are 19 samples-microsat combos that are in the replace list but not in the list of things that need replacing. 
nrow(geno08NA[!(geno08NA$RxnID %in% replace08$RxnID),] )
        
geno08NA[!(geno08NA$RxnID %in% replace08$RxnID),] %>% group_by(pop,microsat) 
head(replace08)
...
head(geno08NA)
dim(geno08NA)
#Here he makes a list of the missing genos from the first dataset. 
...
read.csv("/Users/Maggie/Dropbox/AndyWong/Current R actives/geno08.csv", header=TRUE)->geno08
head(geno08)
#geno08$X<-NULL
20*14
...
geno08new[which(geno08new$allele1!='NA'),]->geno08new

head(geno08NA)
dim(geno08new)#2 out of 2502 rows are incomplete. Thus, we have a success rate of 1-2/2502
geno08NA
geno08new->geno08
head(geno08)
...
dim(geno08)
#AT this stage AW still has 20 samples per 14 plots. 
...
geno08[,c(1,13,2:8)]->geno08 # here we trim data to necessary style.-MPB fix

head(geno08[,c(1,13,2:8)])
dim(geno08)

geno08[,c(1:8)]->geno1#MPB update col codes
geno08[,c(1:7,9)]->geno2 #MPB update col codes
head(geno1)
...
dim(geno1)
dim(geno)
#if there 50000 haploid calls for 2 ploids for 20 plants in 14 sites: each microsat should have 280 plants 
#screened for each microsat:
20*14
(length(geno$microsat)/2)/9
head(geno)
...
genoB=read.csv("/Users/Maggie/Dropbox/AndyWong/1. Hierarchical Structure/data/Genotype processing/genoB.csv", header=TRUE)

head(genoB)
genoB %>% group_by(microsat) %>% summarise(n=n())
genoB$X<-NULL
dim(genoB)
1-20/5040
